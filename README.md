...
```bash
docker compose up -d postgres redis minio minio-init  # PostgreSQL, Redis, MinIO
```
...
# 로컬 기본값은 Docker Compose 서비스와 맞춰져 있습니다.
cp .env.example .env
...
| attack_angle | ° | 임팩트 순간 배트 경로 각도. API/report 노출값은 참조 범위(+5~+25°) 비교용 양수 magnitude이며, 내부 low-level 계산기는 signed diagnostic angle을 유지 |
| hip_shoulder_separation | ° | 골반-어깨 최대 분리각 (정면 촬영 시만) |
| hand_path_efficiency | ratio | 핸드 패스 효율 (0.0~1.0) |
| kinematic_chain | °/s | 각 관절 최대 각속도 |
| stride_length | cm | rotation 시작 시 양발 간격 |
| cog_sway / cog_drop | cm | 무게중심 좌우 흔들림 / 수직 하강 |
| head_stability | cm | 스윙 중 머리 최대 이탈 거리 |
| front_knee_flexion | ° | stride 착지 시 앞무릎 각도 |
| spine_angle | ° | load 시 척추-수직선 각도 |
...
> ⚠️ 측면 촬영 시 회전 분석(hip_shoulder_separation)이 제한됩니다.
> ⚠️ 세로 영상도 지원되지만, 전신이 보이는 가로 영상에서 최적의 결과를 얻을 수 있습니다.
...
│   │   ├── pose_estimator.py
│   │   ├── pose_tracker.py
│   │   ├── batter_identifier.py
│   │   ├── wrist_bat_estimator.py
│   │   ├── bat_detector.py        # legacy/test detector helpers
│   │   ├── bat_tracker.py         # legacy detector trajectory helpers
│   │   ├── trajectory_predictor.py # legacy detector trajectory helpers
│   │   ├── swing_classifier.py
