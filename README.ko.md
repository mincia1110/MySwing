...
> 현재 프로젝트는 로컬 개발/검증용 Docker Compose 구성을 기준으로 합니다. 실제 배포 환경에서는 저장소, CORS, 파일 공개 범위와 신뢰할 수 있는 인증 프록시를 설정해야 하며, 프록시 미설정 시 API는 기본적으로 요청을 차단합니다.
...
`.env.example`은 로컬 UI 실행을 위해 `MYSWING_AUTH_MODE=development`를 명시적으로 켭니다. 이 모드는 서명 없는 `X-User-Id`를 허용하고 헤더가 없으면 고정 개발 사용자를 사용하므로 외부에 공개하면 안 됩니다.

기본값인 `signed_proxy` 모드는 32바이트 이상의 무작위 `MYSWING_AUTH_PROXY_SECRET`과, 사용자 인증을 마친 신뢰 프록시가 생성한 다음 헤더를 요구합니다.

- `X-User-Id`: 정규화된 UUID
- `X-MySwing-Auth-Timestamp`: 현재 Unix timestamp(초)
- `X-MySwing-Auth-Signature`: 소문자 HMAC-SHA256 hex digest

서명할 byte payload는 `timestamp + "\n" + HTTP_METHOD + "\n" + request_path + "\n" + canonical_user_uuid`입니다. 프록시는 외부 요청에 실린 동일 헤더를 모두 제거한 뒤 다시 만들어야 하며, HTTPS와 시간 동기화를 사용해야 합니다. 기본 만료 시간은 60초이고 `MYSWING_AUTH_SIGNATURE_TTL_SECONDS`로 최대 300초까지 설정할 수 있습니다.
...
MySwing은 긴 영상 분석이 아니라 **영상 하나에 스윙 한 번**이 담긴 짧은 클립 분석에 최적화되어 있습니다.

- 권장 클립 길이: **3~7초**
- 이상적인 클립 길이: **약 5초**
- 하드 최대 길이: **10초**. 더 긴 영상은 분석 작업 큐에 들어가기 전에 거절됩니다.
- 긴 영상, 배팅 연습 세션, 전체 경기 클립은 지원하지 않습니다.
- 허용된 짧은 클립에서 강한 스윙성 움직임이 여러 구간 탐지되면 최초 후보만 자동 격리해 분석하고, 후보와 선택 창 근거를 `swing_phases_data.swing_window`에 기록합니다.
- 이는 긴 영상 분할 기능이 아니라 보수적인 안전장치입니다. 가장 안정적인 결과를 위해 업로드 전에 한 번의 스윙만 남기도록 잘라 주세요.
...
| 조명 | 밝은 환경 (40 lux 이상) |
| 프레이밍 | 전신이 보이도록 (머리~발목) |
| 길이 | 영상당 스윙 1회. 3~7초 권장, 약 5초 이상적, 최대 10초 |
| 파일 형식 | MP4, MOV, AVI |
...
- 배트 궤적은 실제 물리적 배트를 직접 감지하는 것이 아니라 pose landmark(손목/팔꿈치) 기반으로 추정합니다.
- 메트릭은 근사값이며 전문 트래킹 장비의 측정값처럼 취급해서는 안 됩니다.
- 프레이밍 불량, 가림, 어두운 조명, 낮은 FPS, 여러 사람이 등장하는 영상은 결과 품질을 떨어뜨릴 수 있습니다.
- 긴 영상에서 스윙 구간을 자동 추출하는 기능은 현재 지원하지 않습니다. 한 번의 스윙만 담긴 짧은 클립을 업로드하세요.
